...
original_metadata = {
    ...
    "creators": [
        {
            ...
            "nameIdentifiers": [
                {
                    "nameIdentifier": "0009-0003-5640-6691",
    ...
    "descriptions": [
        {"description": "A data set of forest fires", "descriptionType": "Summary"}
    ],
...
class TestSubjects(unittest.TestCase):
    ...
    def test_subject_changes(self):
        ...
        record_id = caltechdata_write(
            metadata=test_data, production=False, publish=True
        )
        # Verify correction
        self.assertEqual(
            all_corrected(record_id),
            True,
            f"Subjects in record {record_id} were not corrected properly",
        )
        ...
        record_id = caltechdata_write(
            metadata=copy.deepcopy(original_metadata), production=False, publish=True
        )
        self.assertEqual(
            all_corrected(record_id),
            True,
            f"Subjects in original record {record_id} were not edited properly",
        )
        self.assertEqual(test_change(record_id), True)
        print("Passed test_subject_changes")
        ...
        for subject_obj in record_metadata.get("subjects", []):
            if subject_obj["subject"] in [
                ...
                self.assertIn(
                    "id",
                    subject_obj,
                    f"Subject '{subject_obj['subject']}' in record {record_id} should have an ID",
                )
    ...
    def test_subject_scheme_consistent(self):
        ...
        record_metadata = get_metadata(record_id, production=False)
        for subject_obj in record_metadata.get("subjects", []):
